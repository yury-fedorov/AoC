  ...
  while (getline(f, line)) {
    if (regex_match(line, what, re)) {
      ...
      if (isFirstAnswer) {
        ...
        case Left:
          direction = rotate(true, value, direction);
          break;
        case Right:
          direction = rotate(false, value, direction);
          break;
        default:
          assert(false);
        }
        ...
        case Right: {
          if (value == 180) {
            ...
            // we were going to east ( if now left -> nord (positive), right -
            // south (negative) ) we were going to north (positive) ( if now
