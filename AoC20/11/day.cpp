...
inline void set(Field &f, int x, int y, Place value) { f[y][x] = value; }
...
bool isSame(const Field &a, const Field &b) { return a == b; }
  ...
  while (true) {
    ...
    for (int x = 0; x < xn; x++) {
      for (int y = 0; y < yn; y++) {
        ...
        const auto occupied = isFirstAnswer ? countOccupied(field, x, y)
                                            : countOccupied2(field, x, y);
