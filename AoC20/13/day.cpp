...
BusPosList readBusPosList(const Input &buses, bool toPrint = false) {
  ...
  sort(list.begin(), list.end(),
       [](const BusPos &a, const BusPos &b) { return b.first < a.first; });
  if (toPrint) {
...
TEST_CASE("Day13", "[13]") {
  ...
  // Part 2
  if (part2("17,x,13,19") != 3'417)
    assert(false);
  if (part2("67,7,59,61") != 754'018)
    assert(false);
  if (part2("67,x,7,59,61") != 779'210)
    assert(false);
  if (part2("67,7,x,59,61") != 1'261'476)
    assert(false);
  if (part2("1789,37,47,1889") != 1'202'161'486)
    assert(false);
