...
bool isValid(const Passport &passport) {
  ...
  static const set<string> OBLIGATORY_FIELDS = {"byr", "iyr", "eyr", "hgt",
                                                "hcl", "ecl", "pid"};
  if (n < OBLIGATORY_FIELDS.size())
    return false; // just because not enough fields
...
bool isValidValues(const Passport &passport) {
  ...
  if (byrNumber < 1920 || byrNumber > 2002 || byr.length() != 4)
    return false;
  ...
  const static set<string> EYE_COLORS = {"amb", "blu", "brn", "gry",
                                         "grn", "hzl", "oth"};
...
TEST_CASE("Day04", "[04]") {
  ...
  const int validCount =
      count_if(passports.cbegin(), passports.cend(), isValid);
  REQUIRE(230 == validCount);

  const int validCount2 =
      count_if(passports.cbegin(), passports.cend(), isValidValues);
