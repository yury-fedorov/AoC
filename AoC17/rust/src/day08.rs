...
enum ComparisonOperator { Equal, NotEqual, Greater, Less, GreaterEqual, LessEqual }

fn to_cmd( text : &str ) -> Command {
    match text { "inc" => Command::Inc, "dec" => Command::Dec, _ => panic!( "Unknown command" ) }
}
...
fn get_r( registers : & HashMap<String,i32>, name : &str ) -> i32 {
    match registers.get(name) { Some(result) => *result, None => 0 }
}
...
pub fn tasks( text : &String ) -> ( i32, i32 ) {
   ...
   let instructions : Vec<Instruction> = text.lines().into_iter()
       .flat_map( |line| RE_LINE.captures_iter(line) )
       .map( |cap| ( cap[1].to_string(), to_cmd(&cap[2]), i32::from_str(&cap[3]).unwrap(),
                    cap[4].to_string(), to_op(&cap[5]), i32::from_str(&cap[6]).unwrap() ) )
       .collect();
    ...
    for i in instructions {
        ...
        if new_max_r.is_some() {
            if max_r_ever.is_none() || ( max_r_ever.is_some() && max_r_ever.unwrap() < new_max_r.unwrap() ) {
