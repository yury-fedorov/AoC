...
fn run(
    depth_position: &mut HashMap<i32, Position>,
    max_depth: i32,
    all_scanners: &Vec<i32>,
    stop_when_caught: bool,
) -> HashSet<i32> {
...
fn move_scanners(depth_position: &mut HashMap<i32, (i32, i32, i32)>, all_scanners: &Vec<i32>) {
    ...
    for depth in all_scanners {
        ...
        let next_dir = if next_pos > max_range || next_pos < 0 {
            inverse(dir)
        } else {
            dir
        };
...
pub fn input(conf: &str) -> State {
    ...
    let depth_position: HashMap<i32, Position> = map
        .clone()
        .into_iter()
        .map(|e| (e.0, (0, 1, e.1 - 1)))
        .collect();
...
pub fn task1(state: &State) -> i32 {
    ...
    severity(&map, &run(&mut depth_position, *max_depth, scanners, false))
}
...
pub fn task2(state: &State) -> i32 {
    ...
    loop {
        ...
        let c = run(&mut copy, *max_depth, scanners, true);
        if c.is_empty() {
            return t;
        }
