...
int day13(const bool isFirstAnswer) {
  ...
  for (auto i = 0; i < adjPersonCount; i++) {
    for (auto j = i + 1; j < adjPersonCount; j++) {
      ...
      auto ab =
          find_if(facts.cbegin(), facts.cend(), [nameA, nameB](const Fact &f) {
            return get<0>(f) == nameA && get<3>(f) == nameB;
          });
      auto ba =
          find_if(facts.cbegin(), facts.cend(), [nameA, nameB](const Fact &f) {
            return get<0>(f) == nameB && get<3>(f) == nameA;
          });

      // sum the happiness
      auto fh = [](const Fact &f) { return (get<1>(f) ? 1 : -1) * get<2>(f); };
      const int happiness = fh(*ab) + fh(*ba);
      // cout << nameA << " & " << nameB << " " << fh(*ab) << " + " << fh(*ba)
      // << " = " << happiness << endl;
  ...
  // we also need to ignore direction of count (it gives us another reduction of
  // space by 2) to have such filter we consider that the second position
  // (person on the right from the first) is always smaller than the last one
  // (person on the left from the first) 1 2 ... 8 -> is ok 1 8 ... 2 -> is
  // filtered out
