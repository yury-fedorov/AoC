...
auto day16(const bool isFirstAnswer) {
  ...
  const vector<Fact> known{{"children", 3},    {"cats", 7},   {"samoyeds", 2},
                           {"pomeranians", 3}, {"akitas", 0}, {"vizslas", 0},
                           {"goldfish", 5},    {"trees", 3},  {"cars", 2},
                           {"perfumes", 1}};

  const map<string, function<bool(int, int)>> notExact{
      {"cats", greater<int>()},
      {"trees", greater<int>()},
      {"pomeranians", less<int>()},
      {"goldfish", less<int>()}};

  for (const auto &ck : known) {
    SueFacts good;
    const auto nei = notExact.find(ck.first);
    const auto match = (isFirstAnswer || nei == notExact.end())
                           ? equal_to<int>()
                           : nei->second;
