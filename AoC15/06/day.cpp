...
auto solution(const bool isFirstAnswer) {
  ...
  while (getline(f, line)) {
    if (regex_match(line, what, re)) {
      ...
      // cout << what[1] << " " << what[2] << "," << what[3] << " - " << what[4]
      // << "," << what[5] << endl;
      Instruction i;
      i.operation = what[1];
      i.x0 = stoi(what[2]);
    ...
    for (auto x = i.x0; x <= i.x1; x++)
      for (auto y = i.y0; y <= i.y1; y++) {
        ...
        if (isFirstAnswer) {
          ...
          if (i.operation == "turn on")
            grid[x][y] = 1;
          else if (i.operation == "turn off")
            grid[x][y] = 0;
          else if (i.operation == "toggle")
            grid[x][y] = grid[x][y] > 0 ? 0 : 1;
          else
            cerr << "Unexpected operation: " << i.operation << endl;
        } else {
