...
void addAll(const string &input, const Rules &rules,
...
int distance(const string &input, const ToFromRules &rules,
             const vector<string> &toList, int depth) {
  if (input == START)
    return 0;
...
TEST_CASE("Day19", "[19]") {
  ...
  sort(toList.begin(), toList.end(), [](const string &a, const string &b) {
