...
TEST(AoC22, Day08) {
  ...
  absl::flat_hash_map<Point, char> map;
  int y = 0;
  int max_x = 0;
  ...
  const auto get_scenic_score = [&map, max_x,
                                 max_y](const Point &p) -> long long {
    // any tree on a border is visible
    const int x = p.first;
    const int y = p.second;
    if (day08::is_border(x, y, max_x, max_y))
  ...
  for (const auto &[p, _] : map) {
    ...
    answer2 = std::max(answer2, get_scenic_score(p));
  }
  EXPECT_EQ(answer1, 1647);
  EXPECT_EQ(answer2, 392080);
}
