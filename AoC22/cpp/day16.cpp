...
void RemoveOpened(Doors &doors, const Doors &open) noexcept {
  ...
  absl::c_for_each(open, [&doors](const auto &door) {
    ...
    //    doors.end());
    const auto i = absl::c_find(doors, door);
    if ( i != doors.end() ) doors.erase(i);
  });
}
...
[[nodiscard]] long Pressure2(const Map &map, State actor_a, State actor_b,
  ...
  if ( doors.empty() ) return t * last_minute;
  ...
  for (const auto &target_ai : targets_a) {
    for (const auto &target_bi : targets_b) {
      ...
      if ( targets_a.size() == 1 && targets_b.size() == 1 ) {
        // if only one option in every target, it is ok to go in the same direction
      } else if (target_ai == target_bi && target_ai != kNoDoor)
        continue;
      Doors open1{open};
      const auto step = [&open1, &seq](Door pos, Door target) -> State {
        ...
        if (path.empty()) {
          ...
          if ( target == kNoDoor )
            return State{pos, std::nullopt};
          EXPECT_TRUE(false) << "we are not expected to happen here";
        }
