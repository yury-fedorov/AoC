...
template <class T>
[[nodiscard]] std::optional<T> EvalT(T a, std::string_view op,
                                           T b) noexcept {
  switch (op[0]) {
    case '+':
      return a + b;
    ...
    {
      const auto r = a % b;
      if ( r != 0 ) return std::optional<T>(); // not siutable
      return a / b;
    }
    case '=': 
      return a == b ? 1 : 0;
...
[[nodiscard]] std::optional<Int> Eval(Int a, std::string_view op, Int b) noexcept {
  ...
  EXPECT_TRUE(false) << "Cast cannot be done: " << ToStr(a)
                     << op << ToStr(b) << " = "
                     << ToStr(result);
  return std::optional<Int>();
}

[[nodiscard]] std::optional<Int> Eval(const std::string &node, Map &map) noexcept {
...
[[nodiscard]] std::optional<Var> Eval2(const std::string &node, Map &map) noexcept {
  if ( node == k_human ) return Var { 0, k_human }; // we do formula
