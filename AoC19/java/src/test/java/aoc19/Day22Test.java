...
public class Day22Test {
    ...
    static void dealIntoNewStack(List<Integer> deck) {
    ...
    static class DealIntoNewStack extends Shuffling {
        public DealIntoNewStack(long size) {
            super(size);
        }

        long getNewPosition(long oldPosition) {
            return _size - oldPosition - 1;
        }
    ...
    static class CutCards extends Shuffling {
        ...
        public CutCards(long size, long n) {
            super(size);
            _n = n;
        }

        long makePositive(long value) {
            return value < 0 ? value + _size : value;
        }

        long getNewPosition(long oldPosition) {
            return makePositive((oldPosition - _n) % _size);
        }

        long getOldPosition(long newPosition) {
            return makePositive((newPosition + _n) % _size);
        }
    }
    ...
    static class DealWithIncrement extends Shuffling {
        ...
        public DealWithIncrement(long size, long n) {
            super(size);
            _n = n;
        }

        long getNewPosition(long oldPosition) {
            return oldPosition * _n % _size;
        }
