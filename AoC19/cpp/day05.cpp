...
namespace day05 {
    Number solution( const Memory & code, Number input ) {
        Queue in;
        in.push( input );
        Queue out;
        run(code, in,  out);
    ...
    Number answer1( const Memory & code ) { return solution(code, 1); }
    Number answer2( const Memory & code ) { return solution(code, 5); }
}
